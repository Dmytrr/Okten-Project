...
//
const myFn = (obj, parent) => {
    for (const key in obj) {
        const myDiv = document.createElement('div');
        parent.append(myDiv);
        typeof obj[key] === "object" ? myFn(obj[key], myDiv) : myDiv.textContent = obj[key];
    }
}
myFn(usersList, document.body);
//
//
// --------
...
document.body.append(p1, h1);
p1.append(p2);
p2.append(h2);

const myObj = {headings: [], paragraphs: []};
const rec = document => {
    ...
    for (const element of collection) {
        if(element.tagName === 'H1' ||
            ...
            element.tagName === 'H4' ||
            element.tagName === 'H5' ||
            element.tagName === 'H6' ) myObj.headings.push(element);
        if(element.tagName === 'P' ) myObj.paragraphs.push(element);
        rec(element);
    }
}
rec(document);
